...
import { parts, chapterTitles, getChapter } from './data/chapters.js';
...
const tbPagesMap = {
  1:  ['tb-012-ch01-p1','tb-013-ch01-p2','tb-014-ch01-p3','tb-015-ch01-p4'],
  2:  ['tb-016-ch02-p1','tb-017-ch02-p2','tb-018-ch02-p3','tb-019-ch02-p4'],
  3:  ['tb-020-ch03-p1','tb-021-ch03-p2','tb-022-ch03-p3','tb-023-ch03-p4'],
  4:  ['tb-024-ch04-p1','tb-025-ch04-p2','tb-026-ch04-p3','tb-027-ch04-p4'],
  5:  ['tb-034-ch05-p1','tb-035-ch05-p2','tb-036-ch05-p3','tb-037-ch05-p4'],
  6:  ['tb-038-ch06-p1','tb-039-ch06-p2','tb-040-ch06-p3','tb-041-ch06-p4'],
  7:  ['tb-042-ch07-p1','tb-043-ch07-p2','tb-044-ch07-p3','tb-045-ch07-p4'],
  8:  ['tb-046-ch08-p1','tb-047-ch08-p2','tb-048-ch08-p3','tb-049-ch08-p4'],
  9:  ['tb-056-ch09-p1','tb-057-ch09-p2','tb-058-ch09-p3','tb-059-ch09-p4'],
  10: ['tb-060-ch10-p1','tb-061-ch10-p2','tb-062-ch10-p3','tb-063-ch10-p4'],
  11: ['tb-064-ch11-p1','tb-065-ch11-p2','tb-066-ch11-p3','tb-067-ch11-p4'],
  12: ['tb-068-ch12-p1','tb-069-ch12-p2','tb-070-ch12-p3','tb-071-ch12-p4'],
  13: ['tb-078-ch13-p1','tb-079-ch13-p2','tb-080-ch13-p3','tb-081-ch13-p4'],
  14: ['tb-082-ch14-p1','tb-083-ch14-p2','tb-084-ch14-p3','tb-085-ch14-p4'],
  15: ['tb-086-ch15-p1','tb-087-ch15-p2','tb-088-ch15-p3','tb-089-ch15-p4'],
  16: ['tb-090-ch16-p1','tb-091-ch16-p2','tb-092-ch16-p3','tb-093-ch16-p4'],
  17: ['tb-100-ch17-p1','tb-101-ch17-p2','tb-102-ch17-p3','tb-103-ch17-p4'],
  18: ['tb-104-ch18-p1','tb-105-ch18-p2','tb-106-ch18-p3','tb-107-ch18-p4'],
  19: ['tb-108-ch19-p1','tb-109-ch19-p2','tb-110-ch19-p3','tb-111-ch19-p4'],
  20: ['tb-112-ch20-p1','tb-113-ch20-p2','tb-114-ch20-p3','tb-115-ch20-p4'],
};
const tbImg = (name) => `${import.meta.env.BASE_URL}img/${name}.png`;
...
const wbPagesMap = {
  3: [1,2],    4: [3,4],                                  // Part 1 ch.3-4; dividers: 5,6
  5: [7,8],    6: [9,10],   7: [11,12],  8: [13,14],     // Part 2 ch.5-8; dividers: 15,16
  9: [17,18],  10: [19,20], 11: [21,22], 12: [23,24],    // Part 3 ch.9-12; dividers: 25,26
  13: [27,28], 14: [29,30], 15: [31,32], 16: [33,34],    // Part 4 ch.13-16; dividers: 35,36
  17: [37,38], 18: [39,40], 19: [41,42], 20: [43,44],    // Part 5 ch.17-20; 45-47: back matter
};
const wbImg = (n) => `${import.meta.env.BASE_URL}img/wb-${String(13+n).padStart(3,'0')}-${String(n).padStart(3,'0')}.png`;
...
const prTbPagesMap = {
  1: ['tb-028-p1-review','tb-029-p1-test','tb-030-p1-culture','tb-031-p1-culture-p2'],
  2: ['tb-050-p2-review','tb-051-p2-test-p1','tb-052-p2-test-p2','tb-053-p2-test-p3'],
  3: ['tb-072-p3-review','tb-073-p3-test','tb-074-p3-culture-p1','tb-075-p3-culture-p2'],
  4: ['tb-094-p4-review','tb-095-p4-test','tb-096-p4-culture-p1','tb-097-p4-culture-p2'],
  5: ['tb-116-p5-review','tb-117-p5-test','tb-118-p5-culture-p1','tb-119-p5-culture-p2'],
};
...
const Flashcards = ({ chapter, showRussian, isDark }) => {
...
const App = () => {
  ...
  const progressPct = Math.round((1 / 20) * 100);
    ...
    <div className={`flex h-screen font-sans overflow-hidden transition-colors duration-300 ${themeClasses}`}>
      ...
      <aside className={`fixed md:static inset-y-0 left-0 z-40 md:z-auto h-full flex flex-col flex-shrink-0 transition-all duration-300 ${isSidebarOpen ? 'w-72 translate-x-0' : '-translate-x-full md:translate-x-0 md:w-20'} ${isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'} border-r`}>
        <div className={`p-4 border-b flex items-center justify-between ${isDarkMode ? 'border-zinc-800' : 'border-slate-100'}`}>
          {isSidebarOpen && (
            <div>
              <h1 className={`font-black text-lg leading-tight ${isDarkMode ? 'text-zinc-300' : 'text-blue-500'}`}>KIIP Level 5</h1>
              <p className="text-[10px] uppercase tracking-widest text-zinc-500 font-bold">사회통합프로그램 심화</p>
            </div>
          )}
          <button
        ...
        <div className={`p-4 m-2 rounded-xl ${isDarkMode ? 'bg-zinc-800' : 'bg-slate-100'}`}>
          ...
          {isSidebarOpen && <div className="text-xs mt-1 text-zinc-400 italic">{progressPct}% подготовлено · 1 / 20</div>}
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div ref={mainScrollRef} className="flex-1 overflow-y-auto p-6 md:p-8 max-w-5xl mx-auto w-full">
          ...
          {!activeReviewPart && activeTab === 'flashcards' && <Flashcards chapter={chapter} showRussian={showRussian} isDark={isDarkMode} />}
