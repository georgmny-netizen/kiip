...
const ExamSimulation = ({ isDark, showRussian }) => {
  ...
  const [sel, setSel] = useState({ quiz: true, speaking: true });
  ...
  }, [phase, quizTime, speakingTime]);

  const loadExam = async () => {
    setPhase('loading');
    const all = await Promise.all(Array.from({ length: 50 }, (_, i) => getChapter(i + 1)));
    let allQuiz = [], allSpeaking = [];
    all.forEach((ch, i) => {
      if (!ch) return;
      const n = i + 1;
  ...
  if (phase === 'intro') {
    const sections = [
      { key: 'quiz',     label: '객관식', ru: 'Тест с выбором ответа', sub: '36문항 · 50분', pts: '72점', color: 'blue',    ring: 'ring-blue-500',    bg: 'bg-blue-500' },
      { key: 'speaking', label: '구술형', ru: 'Устная часть',          sub: '5문항 · 10분',  pts: '28점', color: 'emerald', ring: 'ring-emerald-500', bg: 'bg-emerald-500' },
    ];
    const anySelected = sel.quiz || sel.speaking;
    return (
      <div className="space-y-6 animate-in fade-in duration-500 max-w-2xl mx-auto">
        <div className={`${card} text-center`}>
          <div className="text-5xl mb-3">🎓</div>
          <h2 className={`text-2xl font-black mb-1 ${isDark ? 'text-zinc-200' : 'text-slate-800'}`}>BasicUOKS</h2>
          <p className={`text-sm font-medium mb-5 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>Выберите разделы</p>
          ...
          <p className={`text-xs mb-6 ${isDark ? 'text-zinc-500' : 'text-slate-400'}`}>
            Итого: <b>100 баллов</b> · Проходной: <b>60 баллов</b> · Общее время: <b>60 минут</b>
          </p>
          ...
          <button onClick={loadExam} disabled={!anySelected}
            ...
            {sel.quiz && sel.speaking ? 'Полный экзамен' : 'Начать тренировку'} →
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-4 animate-in fade-in duration-300">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {!isWB && quizIdx === 35 && (
            <button onClick={goToSpeaking}
              className="ml-auto flex items-center gap-1 px-5 py-2.5 rounded-xl font-bold text-sm bg-emerald-600 hover:bg-emerald-500 text-white transition-all">
              Завершить → 구술형 <ChevronRight size={16} />
            </button>
          )}
          {isWB && (
            <button onClick={() => quizIdx < 35 ? setQuizIdx(i => i + 1) : goToSpeaking()}
              disabled={!revealed}
  ...
  if (phase === 'results') {
    ...
    const tot = (qs ?? 0) + (ss ?? 0);
    const maxPts = (sel.quiz ? 72 : 0) + (sel.speaking ? 28 : 0);
    const fullExam = sel.quiz && sel.speaking;
    const passed = fullExam && tot >= 60;
    const allSections = [
      { key: 'quiz',     label: '객관식', score: qs, max: 72, color: 'blue' },
      { key: 'speaking', label: '구술형', score: ss, max: 28, color: 'emerald' },
    ].filter(s => sel[s.key]);
    return (
      <div className="space-y-6 animate-in fade-in duration-500">
        ...
        {sel.speaking && <div className={card}>
          <h3 className={`font-black text-base mb-4 ${isDark ? 'text-zinc-200' : 'text-slate-800'}`}>Разбор 구술형</h3>
          <div className="space-y-3">
            {examData.speakingQs.map((q, i) => (
              <div key={i} className={`p-3 rounded-xl text-sm ${isDark ? 'bg-zinc-800 border border-zinc-700' : 'bg-slate-50 border border-slate-200'}`}>
                <p className={`font-medium mb-2 ${isDark ? 'text-zinc-300' : 'text-slate-700'}`}>{i + 1}. {showRussian && q.ruQuestion ? q.ruQuestion : q.question}</p>
                <p className={`text-xs mb-2 ${isDark ? 'text-zinc-400' : 'text-slate-500'}`}>Ваш ответ: {speakingTexts[i] || <i>не написан</i>}</p>
                <p className={`text-xs font-black ${isDark ? 'text-emerald-400' : 'text-emerald-600'}`}>Оценка: {speakingScores[i] ?? 0} / 5</p>
                {q.hints && (
                  <div className={`mt-2 p-2 rounded-lg text-xs ${isDark ? 'bg-zinc-900 text-zinc-400' : 'bg-white text-slate-500'}`}>
                    <p className="font-bold mb-1">Ключевые пункты:</p>
                    {q.hints.map((h, j) => <p key={j}>• {h}</p>)}
                  </div>
                )}
              </div>
            ))}
